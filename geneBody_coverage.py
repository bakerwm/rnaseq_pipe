#!/home/wangm/miniconda3/envs/rseqc/bin/python
'''
	...
	with open(refbed) as r:
		for line in r:
			try:
				...
				geneName = fields[3]
				strand = fields[5]
				geneID = '_'.join(list(map(str, [chrom, tx_start, tx_end, geneName, strand])))
...
def main():
	...
	#---------------------------------------------------------------------------------------------------------------------------------------#
	gbcov_txt = options.output_prefix  + ".geneBodyCoverage.txt"
	# header
	header = list(map(str, ['Percentile'] + list(range(1,101))))
	...
	for bamfile in bamfiles:
		...
		cvg = genebody_coverage(bamfile, gene_percentiles) #!!! to-do
		if len(cvg) == 0:
